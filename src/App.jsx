...
const AuthModal = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  const handlePhoneSubmit = async (e) => {
    ...
    if (Capacitor.isNativePlatform()) {
      try {
        console.log('Starting native phone auth for:', formData.phone_number);
        ...
        if (vId) {
          ...
          console.error('Missing verificationId in result:', result);
          throw new Error('Firebase did not return a verification ID. Please check Logcat for native errors.');
        }
      } catch (err) {
        console.error('Native Auth Error:', err);
        setError(err.message || 'Native authentication failed');
      } finally {
        setLoading(false);
      }
      return;
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      ...
      >
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.9rem', color: 'hsl(var(--muted-foreground))' }}>
          ...
          <span onClick={() => { if (!loading) { setIsLogin(!isLogin); setAuthMethod('email'); } }} style={{ color: 'hsl(var(--primary))', fontWeight: 700, cursor: loading ? 'default' : 'pointer', opacity: loading ? 0.5 : 1 }}>
